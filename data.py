...
class DataGeneration(Dataset):
    ...
    对文本进行处理
    """
    def __init__(self,bert_tokenizer,data,max_char_len=512):
        """
        bert_tokenizer:分词器
        data：语料（二维数组）[摘要,定义,label]
        """
        self.bert_tokenizer = bert_tokenizer
        self.max_seq_len = max_char_len
        self.seqs, self.seq_masks ,self.seq_segments , self.labels = self.get_input(data)
    ...
    def get_input(self, array):
        ...
        入参:
            dataArray     : 二维数组格式，包含三列，第一,二列为文本，第三列为标签。标签取值为{0,1}，其中0表示负样本，1代表正样本。
        ...
        """
        sentences_1 = [x[0] for x in array]
        sentences_2 = [x[1] for x in array]
        labels = [x[2] for x in array]
    ...
    def trunate_and_pad(self,tokens_seq_1, tokens_seq_2):
        """
         1. 如果是单句序列，按照BERT中的序列处理方式，需要在输入序列头尾分别拼接特殊字符'CLS'与'SEP'，
                   因此不包含两个特殊字符的序列长度应该小于等于max_seq_len-2，如果序列长度大于该值则进行截断。
        ...
        """
        # 对超长序列进行截断 abs与def比例为4：1
        if len(tokens_seq_1) > ((self.max_seq_len - 3)*4 // 5):
            tokens_seq_1 = tokens_seq_1[0:(self.max_seq_len - 3) // 2]
        if len(tokens_seq_2) > ((self.max_seq_len - 3) // 5):
